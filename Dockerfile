FROM debian:jessie
RUN apt-get update -y && apt-get install -y build-essential dh-autoreconf unzip wget libpcre3 libpcre3-dev zlib1g zlib1g.dev
RUN mkdir ~/nginx-dev
RUN cd ~/nginx-dev \
    && wget https://github.com/chrislim2888/IP2Location-C-Library/archive/master.zip \
    && unzip master.zip && rm master.zip\
    && cd IP2Location-C-Library-master \
    && autoreconf -i -v --force \
    && ./configure \
    && make \
    && make install \
    && ldconfig
RUN cd ~/nginx-dev \
    && wget https://github.com/ip2location/ip2location-nginx/archive/master.zip \
    && unzip master.zip && rm master.zip
COPY nginx.conf /etc/nginx/nginx.conf
RUN cd ~/nginx-dev \
    && wget http://nginx.org/download/nginx-1.14.1.tar.gz \
    && tar xvfz nginx-*.tar.gz && rm nginx-*.tar.gz \
    && cd ~/nginx-dev/nginx-1.14.1 \
    && ./configure \
        --conf-path=/etc/nginx/nginx.conf \
        --add-module=/root/nginx-dev/ip2location-nginx-master \
        --with-http_realip_module \
    && make \
    && make install
COPY DB11.BIN /etc/ip2location/DB11.BIN
EXPOSE 8080
STOPSIGNAL SIGTERM
CMD ["/usr/local/nginx/sbin/nginx", "-g", "daemon off;"]
